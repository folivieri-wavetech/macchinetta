...
import time
import datetime
from hyper_gold_engine import HyperGoldEngine, CANDLE_SECONDS, WARMUP_BARS_KJ, WARMUP_BARS_TK, CORE_CONTRACTS, INC_CONTRACTS, MAX_INCREMENTS, INC_TP_PIPS, CORE_TP_PIPS

# Configurazione Pagina Streamlit
st.set_page_config(
    page_title="Hyper-Trading Spot Gold 1€ (30S)",
    page_icon="⚡",
    layout="wide",
...
def render_live_desk():
    ...
    # Determinazione Regime Macro TK233
    if live_mid and tk:
        if live_mid > tk:
            regime_label = "🟢 BULLISH (SOLO LONG)"
            regime_col = "#22c55e"
        elif live_mid < tk:
            regime_label = "🔴 BEARISH (SOLO SHORT)"
            regime_col = "#ef4444"
        else:
            regime_label = "⚪ NEUTRALE"
            regime_col = "#94a3b8"
    else:
        regime_label = "⏳ CALCOLO TK233..."
        regime_col = "#cbd5e1"

    # HEADER SUPERIORE
    c_title, c_badges = st.columns([2.5, 1.5])
    with c_title:
        st.markdown("## ⚡ Hyper-Trading Spot Gold 1€ (Barre 30s • TK233 / KJ55)")
        st.caption("Filtro Macro TK 233 • Trigger KJ 55 • Core 5c + Max 5 Incr x 3c (TP +2 pip) • Totale max 20c • Porta 8504")
    ...
    with m2:
        ...
        st.markdown(f"""
        <div class='kpi-card' style='padding: 10px 14px;'>
            <div class='kpi-title'>Livelli Chiave (30s) • <span style='color: {regime_col}; font-weight: bold;'>{regime_label}</span></div>
            <div style='display: flex; gap: 16px; align-items: baseline; margin-top: 4px;'>
                ...
                <div>
                    <span style='font-size: 0.75rem; color: #94a3b8;'>KJ 55: </span>
                    <span style='font-size: 1.15rem; font-weight: 700; color: #FFD700;'>{kj_str}</span>
                </div>
                <div>
                    <span style='font-size: 0.75rem; color: #94a3b8;'>TK 233: </span>
                    <span style='font-size: 1.15rem; font-weight: 700; color: #f97316;'>{tk_str}</span>
                </div>
            </div>
            <div style='font-size: 0.73rem; color: #94a3b8; margin-top: 3px;'>Filtro: Prezzo > TK233 solo Long • Prezzo < TK233 solo Short</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if candles_count < WARMUP_BARS_TK:
        ...
        st.info(f"⏳ **Accumulo Barre 30s In Corso:** {candles_count} / {WARMUP_BARS_TK} barre raccolte. Mancano {max(0, WARMUP_BARS_TK - candles_count)} barre ({max(0, (WARMUP_BARS_TK - candles_count) * 30 // 60)} min) per il calcolo completo della TK 233.")
        st.progress(pct_warmup)
    else:
        st.success(f"✅ **Indicatori 30s Pienamente Operativi:** {candles_count} barre storiche disponibili. KJ55 e TK233 calcolate in tempo reale.")
    ...
    with col_btn1:
        if st.button("🟢 AVVIA TRADING", use_container_width=True, type="primary" if not trading_on else "secondary"):
            engine.set_trading(True)
            st.rerun()

    with col_btn2:
        if st.button("🛑 STOP TRADING", use_container_width=True, type="primary" if trading_on else "secondary"):
    ...
    with col_t1:
        st.markdown("### 📋 Storico Operazioni Eseguite (30s • TK233 / KJ55)")
        if trades:
            rows_html = []
            for t in trades[:30]:
            ...
            st.info("Nessuna operazione ancora eseguita. Al primo segnale conforme al filtro TK233 e trigger KJ55 verrà registrato l'ordine.")

    with col_t2:
        st.markdown("### ⏱️ Ultime Barre Concluse (30s)")
        if candles_recent:
            rows_c = []
            ...
            st.info("In attesa della prima barra da 30 secondi...")
