...
import copy
with open(FILE_MEMORIA, "r") as f: MEMORIA_IN_RAM = json.load(f)
# Disabilita le print verbose per velocizzare
import builtins
    ...
    if stato == "IN_ATTESA":
        ...
        if not core_trend:
            ...
            if not vittoria:
                ...
                # --- FIX: TRADUZIONE DIREZIONE E RIPARTENZA AUTOMATICA ---
                nuova_dir = "LONG" if sat_dir == "BUY" else "SHORT"
                aggiorna_memoria(nome, {"stato": "IN_ATTESA", "tentativi_sat": 0, "attivo": True, "direzione": nuova_dir})
                # ---------------------------------------------------------
...
def main_backtest():
    # --- INSERISCI QUI IL NOME ESATTO DEL TUO FILE CSV ---
    file_csv = "C:/Users/Fiordok/Desktop/Macchinetta_IG/AUDNZD_1m_data.csv" 
    strumento = "AUD/NZD"
    
    # Parametri sacri impostati come da tua richiesta
    MEMORIA_IN_RAM[strumento] = {
        "attivo": True,
        "direzione": "LONG",
        "tp": 90,
        "opp": 15,
    ...
    # Ho impostato le date molto larghe per leggere tutto il file a prescindere dall'anno
    ...
    try:
        with open(file_csv, "r", encoding="utf-8") as f:
            ...
            conteggio = 0
            builtins.print(f"🚀 Inizio Backtest per {strumento}...")
            
            max_drawdown = 0.0
            
            for row in reader:
                # row: Date;Open;High;Low;Close;Volume
                # Formato Date: 2004.06.11 07:18
                ...
                # Leggiamo i 4 prezzi della candela a 1 minuto
                prezzi = [float(row[1]), float(row[2]), float(row[3]), float(row[4])] 
                
                for p in prezzi:
                    verifica_trigger_broker(p, epic, mult)
                    
                    stato_precedente = None
                    while True:
                        stato_attuale = MEMORIA_IN_RAM.get(strumento, {}).get("stato", "")
                        if stato_attuale == stato_precedente:
                            break
                        stato_precedente = stato_attuale
                        logica_motore_finto(strumento, p)
                
                # Calcolo Max Drawdown
                posizioni = [pos for pos in BROKER_POSIZIONI if pos['market']['epic'] == epic]
                floating_totale = 0.0
                for pos in posizioni:
                    sz = float(pos['position']['size'])
                    lvl = float(pos['position']['level'])
                    direz = pos['position']['direction']
                    if direz == "BUY":
                        floating_totale += ((prezzi[-1] - lvl) / mult) * sz
                    else:
                        floating_totale += ((lvl - prezzi[-1]) / mult) * sz
                
                if floating_totale < max_drawdown:
                    max_drawdown = floating_totale
                
                conteggio += 1
                if conteggio % 10000 == 0:
                    builtins.print(f"Elaborate {conteggio} righe (Data: {date_str}) -> Cassa: {CASSA_SIMULATA:+.2f} € | Max DD: {max_drawdown:+.2f} €")
                    
    except FileNotFoundError:
        builtins.print(f"❌ File {file_csv} non trovato. Controlla il percorso o il nome.")
        return

    builtins.print("\n" + "="*50)
    builtins.print("🏁 BACKTEST COMPLETATO")
    builtins.print("="*50)
    builtins.print(f"📊 Strumento elaborato: {strumento}")
    builtins.print(f"⚙️ Parametri: TP=90 | OPP=15 | DTS=15 | SIZE=10")
    builtins.print(f"📅 Righe (minuti) elaborate: {conteggio}")
    builtins.print(f"🏦 CASSA FINALE (PnL): {CASSA_SIMULATA:+.2f} €")
    builtins.print(f"📉 MAX DRAWDOWN:      {max_drawdown:+.2f} €")
    builtins.print(f"📦 Posizioni Rimaste: {len(BROKER_POSIZIONI)}")
    builtins.print("="*50 + "\n")
