...
for acc in ["FIORDOK_DEMO", "DANY_DEMO", "BONGIOLO_DEMO"]:
    ...
    if os.path.exists(stato_file):
        with open(stato_file) as fp:
            px_gold = json.load(fp).get("prezzi_live", {}).get("Spot Gold", 4426.5)
            
    print(f"\n================ SPOT GOLD PER {acc} (Live IG = {px_gold}) ================")
    for tf in tfs:
        ...
        if len(candele) >= 55:
            ...
            print(f"  [{tf:8s}] {len(candele):3d} candele (Da {sub55[0]['snapshotTime']} a {sub55[-1]['snapshotTime']})")
            print(f"            Max: {max_h:.1f} | Min: {min_l:.1f} | ➡️ KJ55: {kj:.1f} | Distanza: {abs(diff):.1f} pt ({dir_pos})")
        else:
            print(f"  [{tf:8s}] Meno di 55 candele ({len(candele)})")
    ...
    print(f"✅ Radar Trend ricalcolato per {acc}")
