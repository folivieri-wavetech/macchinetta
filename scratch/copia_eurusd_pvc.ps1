...
foreach ($acc in $accounts) {
    foreach ($tf in $timeframes) {
        $src = "FIORDOK_DEMO/candele_EUR_USD_${tf}.json"
        $dst = "${dashPod}:/data/${acc}/candele_EUR_USD_${tf}.json"
...
Write-Host "Copia ed esecuzione applica_eurusd_server.py..."
.\kubectl.exe --kubeconfig=.\local.yaml -n macchinetta cp scratch/applica_eurusd_server.py "${dashPod}:/data/applica_eurusd_server.py" -c dashboard
.\kubectl.exe --kubeconfig=.\local.yaml -n macchinetta exec deploy/macchinetta-dashboard -c dashboard -- python3 /data/applica_eurusd_server.py
